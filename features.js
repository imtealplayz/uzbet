...
async function handleAffiliateMemberJoin(member, client, cachedInvitesBefore, invitesAfter, usedInviteCode) {
  try {
    ...
    const referrerId = affiliateInvites[code];
    if (!referrerId) return false;

    // Make sure it's actually an affiliate invite
    const ownership = getInviteOwnership(code);
    if (ownership && ownership !== "affiliate") return false;
    ...
    saveDB(db);
    console.log(`✅ [Affiliate] ${member.user.username} matched affiliate invite from ${referrerId}`);
    ...
    if (referrer) referrer.send({ embeds: [baseEmbed("🤝 New Referral!", 0x2ECC71)
      .setDescription(`**${member.user.username}** joined via your affiliate link!\n\nYou'll earn **${AFF_PCT_DISPLAY}** of their wagers when they verify.`)] }).catch(() => {});
    return true;
...
async function cmdPrizepoolPanel(interaction, client) {
  ...
  if (!ch) return;
  const msg = await ch.send({ embeds: [buildPrizepoolEmbed(pool)], components: [row] });
...
async function handlePrizepoolMemberJoin(member, client, cachedInvitesBefore, invitesAfter, usedInviteCode) {
  try {
    const guildId = member.guild.id;
    const userId  = member.user.id;
    ...
    const referrerId = ppInvites[code];
    if (!referrerId) return false;

    const ownership = getInviteOwnership(code);
    if (ownership && ownership !== "prizepool") return false;
...
async function handleCodeMemberJoin(member, client, cachedInvitesBefore, invitesAfter, usedInviteCode) {
  try {
    const cd = getGlobal("guessCode");
    ...
    const inviterId   = codeInvites[code];
    ...
    db2[guildId][userId].guessCodeReferredBy = inviterId;
    saveDB(db2);
    setGlobal("guessCode", cd);
...
async function cmdDeposit(interaction, guildId, targetUser, amount, client) {
  ...
  // ── Public deposit log ──
  const depositLogId = getDepositLogChannel();
  if (depositLogId) {
    try {
      const logCh = await client.channels.fetch(depositLogId).catch(() => null);
      if (logCh) {
        const discordUser = await client.users.fetch(targetUser.id).catch(() => null);
        const logEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "👤 User",      value: `<@${targetUser.id}>`,                   inline: true },
            { name: "💵 Amount",    value: `${finalAmount.toLocaleString()} ${R}`,  inline: true }
          )
          .setTimestamp()
          .setFooter({ text: "🎰 Casino Bot • Deposit Logs" });
...
async function cmdWithdrawPanel(interaction) {
  ...
  const wch = interaction.channel || await interaction.client.channels.fetch(interaction.channelId).catch(() => null);
  if (wch) await wch.send({ embeds: [embed] });
}

// ── /admin setlogchannel ──────────────────────────────────────────────────────

function getDepositLogChannel()  { return getGlobal("depositLogChannelId")  ?? null; }
function getWithdrawLogChannel() { return getGlobal("withdrawLogChannelId") ?? null; }

async function cmdSetDepositLogChannel(interaction) {
  if (!isOwner(interaction)) return interaction.reply({ content: "❌ No permission.", flags: MessageFlags.Ephemeral });
  const channel = interaction.options.getChannel("channel");
  setGlobal("depositLogChannelId", channel.id);
  await interaction.reply({ content: `✅ Deposit log channel set to <#${channel.id}>.`, flags: MessageFlags.Ephemeral });
}

async function cmdSetWithdrawLogChannel(interaction) {
  if (!isOwner(interaction)) return interaction.reply({ content: "❌ No permission.", flags: MessageFlags.Ephemeral });
  const channel = interaction.options.getChannel("channel");
  setGlobal("withdrawLogChannelId", channel.id);
  await interaction.reply({ content: `✅ Withdrawal log channel set to <#${channel.id}>.`, flags: MessageFlags.Ephemeral });
}
  ...
  // ── Admin: Approve withdrawal ─────────────────────────────────────────────
    ...
    try {
      ...
      if (discordUser) {
        ...
        // ── Public withdrawal log ──
        const wdLogId = getWithdrawLogChannel();
        if (wdLogId) {
          try {
            const logCh = await client.channels.fetch(wdLogId).catch(() => null);
            if (logCh) {
              const logEmbed = new EmbedBuilder()
                .setColor(0x2ECC71)
                ...
                .setThumbnail(discordUser.displayAvatarURL() || null)
                .addFields(
                  { name: "👤 User",   value: `<@${txUser}>`,                           inline: true },
                  { name: "💰 Amount", value: `${tx.amount.toLocaleString()} ${R}`,     inline: true }
                )
                .setTimestamp()
                .setFooter({ text: "🎰 Casino Bot • Withdrawal Logs" });
              await logCh.send({ embeds: [logEmbed] }).catch(() => {});
            }
          } catch { /* log failure is non-critical */ }
        }
